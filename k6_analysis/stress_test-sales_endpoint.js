...
export default function () {
    ...
    check(responses, {
        'retrieve all the database - 1': (resp) => resp[0].json().length > 0,
        'retrieve all the database - 2': (resp) => resp[1].json().length > 0,
        'retrieve all the database - 3': (resp) => resp[2].json().length > 0,
        'retrieve all the database - 4': (resp) => resp[3].json().length > 0,
    });
    sleep(1);
}
